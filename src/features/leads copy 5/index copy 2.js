...
function Crousel() {
  ...
  const fetchBlogs = async () => {
    try {
      const response = await axios.get("http://localhost:8000/crousel/");
  ...
  const handleDelete = async (blogId) => {
    try {
      await axios.delete(`http://localhost:8000/crousel/${blogId}`);
  ...
  const handleUpdate = async (values) => {
    try {
      await axios.put(
        `http://localhost:8000/crousel/${selectedBlog._id}`,
        values
      );
  ...
  const handleCreate = async (values) => {
    try {
      await axios.post("http://localhost:8000/crousel/create", values);
